...
export default function PropertyCard({
  ...
  const shareProperty = async (e) => {
    ...
    const shareUrl =
      `${window.location.origin}/property/${encodeURIComponent(asString(propertyId))}`;
...
`;
  ...
  return (
    <motion.div
      ...
      className="
        group relative mx-0 my-1.5 flex cursor-pointer flex-col
        overflow-hidden rounded-[22px] border border-white/70 bg-white
...
function PropertyImage({
  ...
  return (
    <div className="relative w-full">
      <div className="relative aspect-[12/10] w-full overflow-hidden bg-slate-100 lg:aspect-[4/3]">
...
function PropertyCardSkeleton() {
  return (
    <motion.div
      ...
      "
    >
      <div className="relative aspect-[12/10] overflow-hidden bg-gradient-to-br from-slate-100 via-slate-200/70 to-slate-100 lg:aspect-[4/3]">
        <PageShimmer />
      ...
      <div className="space-y-2.5 px-3 pb-3 pt-3">
        ...
        <SkeletonLine className="h-2.5 w-[55%] rounded-full" />
        <SkeletonLine className="h-2.5 w-[68%] rounded-full" />
      </div>
    </motion.div>
  );
}
...
function IconText({ icon, text, textSize = "text-[8px]" }) {
  return (
    <div className="flex min-w-0 items-center gap-1 truncate text-gray-500">

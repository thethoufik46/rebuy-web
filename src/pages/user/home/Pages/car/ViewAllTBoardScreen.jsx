...
export default function ViewAllTBoardScreen() {
  ...
  // ─── Fetch cars (ONLY "t board") ───────────────────────
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const response = await API.get('/cars', { params: { board: 't board' } });
        const cars = response.data?.cars || [];
        setAllCars(cars);
      } catch (err) {
        console.error('Error fetching T Board cars:', err);
        setAllCars([]);
      } finally {
        setIsLoading(false);
      }
  ...
  }, [selectedBrand, allVariants]);

  // ─── Brand map from T Board cars ─────────────────────
      ...
      <div className="px-3 pt-3 pb-6">
        ...
        {filteredCars.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64">
            ...
            <p className="text-gray-500 font-medium">No T Board cars found</p>
